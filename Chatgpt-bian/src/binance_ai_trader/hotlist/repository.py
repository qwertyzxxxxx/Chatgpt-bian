...
class HotlistWatchlistRepository:
    def __init__(self, database: Path) -> None:
        ...
        for _col in ("stop_loss", "tp1", "tp2", "rr", "expires_at", "rank_type"):
            try:
                default = "'UNKNOWN'" if _col == "rank_type" else "NULL"
                self._connection.execute(
                    f"ALTER TABLE hotlist_alerts ADD COLUMN {_col} TEXT DEFAULT {default}"
                )
            except Exception:
                pass
        self._connection.execute(
        ...
        with self._connection:
            self._connection.execute(
                ...
                INSERT INTO hotlist_alerts(
                    ...
                    stop_loss, tp1, tp2, rr, expires_at, rank_type
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
