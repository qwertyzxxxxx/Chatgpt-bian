...
class HotlistPerformanceRepository:
    ...
    def __init__(self, database: Path) -> None:
        ...
        try:
            self._connection.execute(
                "ALTER TABLE hotlist_opportunities ADD COLUMN rank_type TEXT NOT NULL DEFAULT 'UNKNOWN'"
            )
    ...
    def save_opportunity(
        ...
        self._connection.execute(
            ...
            INSERT OR IGNORE INTO hotlist_opportunities
                (symbol, direction, entry, sl, tp1, tp2, rr, confidence, created_at, expiry, rank_type)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                opportunity.symbol,
    ...
    def _opportunity(row: sqlite3.Row) -> TrackedHotlistOpportunity:
        rank_type = "UNKNOWN"
        try:
            rank_type = str(row["rank_type"]) if row["rank_type"] else "UNKNOWN"
        except Exception:
            pass
