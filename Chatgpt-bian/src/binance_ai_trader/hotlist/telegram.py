...
def format_hotlist_alert_batch_message(
    ...
    Ranking basis (same for all signals in this batch):
      Primary   : |24h 涨跌幅| DESC
      Secondary : 成交量(USDT) DESC
      Tiebreaker: Symbol ASC
    The header shows candidate_count (number of signals pushed this cycle).
    """
    ...
    lines = [
        ...
        f"本次推送: {candidate_count} 个信号  |  排序: |24h涨跌| → 成交量 → 代码",
        "",
    ]
