...
class ProductionRunnerTest(unittest.TestCase):
    ...
    def test_default_schedule_includes_history_and_health_automation(self) -> None:
        callback = lambda: 0
        tasks = default_tasks(
            callback, callback, callback, callback, callback,
            collect_history=callback, health=callback,
        )
        ...
        self.assertEqual(timedelta(minutes=15), schedules["health"].interval)
        self.assertEqual(time(0, 5), schedules["daily_report"].daily_at)
        self.assertEqual(time(0, 20), schedules["collect_history"].daily_at)
