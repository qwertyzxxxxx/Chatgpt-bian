...
class TestItemsForGeminiFilter(unittest.TestCase):
    """PR #33 — validate candidate filtering rules for items_for_gemini."""

    def setUp(self):
        self._db = tempfile.NamedTemporaryFile(suffix=".db", delete=False)
        self.repo = LeaderboardWatchRepository(self._db.name)
    ...
    # helpers
    # ------------------------------------------------------------------
    def _insert(self, symbol: str, rank_type: str = "GAINER",
                position: int = 1, change: str = "15.5",
                volume: str = "5000000.0", status: str = "NEW") -> None:
        item = WatchItem(
            watch_id=f"id-{symbol}",
            symbol=symbol,
            first_seen_at=_now(),
            last_seen_at=_now(),
            first_rank_type=rank_type,
            ...
            best_rank_position=position,
            latest_rank_position=position,
            first_change_24h=change,
            latest_change_24h=change,
            quote_volume=volume,
            appearances_24h=1,
            status=status,
        )
        ...
        if status not in ("NEW", "ACTIVE"):
            self.repo._con.execute(
                "UPDATE leaderboard_watch_items SET status=? WHERE symbol=?",
                (status, symbol),
            )
            self.repo._con.commit()
    ...
    # ------------------------------------------------------------------
    def test_new_item_enters_candidates(self):
        self._insert("NEWCOIN", change="20.0", status="NEW")
        syms = [i.symbol for i in self.repo.items_for_gemini(max_n=20)]
        self.assertIn("NEWCOIN", syms)

    def test_active_item_enters_candidates(self):
        self._insert("ACTIVECOIN", change="20.0", status="ACTIVE")
        syms = [i.symbol for i in self.repo.items_for_gemini(max_n=20)]
        self.assertIn("ACTIVECOIN", syms)

    def test_open_item_excluded(self):
        self._insert("OPENCOIN", change="20.0", status="OPEN")
        syms = [i.symbol for i in self.repo.items_for_gemini(max_n=20)]
        self.assertNotIn("OPENCOIN", syms)

    def test_expired_item_excluded(self):
        self._insert("EXPIREDCOIN", change="20.0", status="EXPIRED")
        syms = [i.symbol for i in self.repo.items_for_gemini(max_n=20)]
        self.assertNotIn("EXPIREDCOIN", syms)

    def test_closed_within_cooldown_excluded(self):
        from datetime import timedelta
        self._insert("CLOSEDCOIN", change="20.0", status="CLOSED")
        # last_seen_at = just now → within 4-h cooldown
        self.repo._con.execute(
            "UPDATE leaderboard_watch_items SET last_seen_at=? WHERE symbol=?",
            (_now(), "CLOSEDCOIN"),
        )
        self.repo._con.commit()
        syms = [i.symbol for i in self.repo.items_for_gemini(max_n=20, closed_cooldown_hours=4.0)]
        self.assertNotIn("CLOSEDCOIN", syms)

    def test_closed_outside_cooldown_allowed(self):
        from datetime import timedelta
        self._insert("OLDCLOSED", change="20.0", status="CLOSED")
        old = (
        ...
        self.repo._con.commit()
        # CLOSED is still not in ('NEW','ACTIVE') → excluded regardless
        syms = [i.symbol for i in self.repo.items_for_gemini(max_n=20, closed_cooldown_hours=4.0)]
        self.assertNotIn("OLDCLOSED", syms)

    # ------------------------------------------------------------------
    # Rule 3+4: movement filter — low volatility excluded
    # ------------------------------------------------------------------
    def test_volume_low_change_excluded_from_candidates(self):
        """VOLUME coin with abs(change) < 8% must NOT enter candidates (BTC/ETH case)."""
        self._insert("BTCUSDT", rank_type="VOLUME", change="-0.853")
        self._insert("ETHUSDT", rank_type="VOLUME", change="-1.716")
        syms = [i.symbol for i in self.repo.items_for_gemini(max_n=20, min_move_pct=8.0)]
        self.assertNotIn("BTCUSDT", syms)
        self.assertNotIn("ETHUSDT", syms)

    def test_volume_high_change_included(self):
        """VOLUME coin with abs(change) >= 8% SHOULD enter candidates."""
        self._insert("HIGHVOLCOIN", rank_type="VOLUME", change="12.0")
        syms = [i.symbol for i in self.repo.items_for_gemini(max_n=20, min_move_pct=8.0)]
        self.assertIn("HIGHVOLCOIN", syms)

    def test_gainer_above_threshold_included(self):
        self._insert("GAINER_A", rank_type="GAINER", change="25.0")
        syms = [i.symbol for i in self.repo.items_for_gemini(max_n=20, min_move_pct=8.0)]
        self.assertIn("GAINER_A", syms)

    def test_loser_above_threshold_included(self):
        self._insert("LOSER_A", rank_type="LOSER", change="-18.0")
        syms = [i.symbol for i in self.repo.items_for_gemini(max_n=20, min_move_pct=8.0)]
        self.assertIn("LOSER_A", syms)

    def test_gainer_below_threshold_excluded(self):
        """Even a GAINER with abs(change) < 8% is excluded."""
        self._insert("SMALLGAINER", rank_type="GAINER", change="5.0")
        syms = [i.symbol for i in self.repo.items_for_gemini(max_n=20, min_move_pct=8.0)]
        self.assertNotIn("SMALLGAINER", syms)

    def test_min_move_pct_exact_boundary(self):
        """Exactly at threshold passes; one cent below fails."""
        self._insert("EXACT8", rank_type="GAINER", change="8.0")
        self._insert("BELOW8", rank_type="GAINER", change="7.99")
        syms = [i.symbol for i in self.repo.items_for_gemini(max_n=20, min_move_pct=8.0)]
        self.assertIn("EXACT8", syms)
        self.assertNotIn("BELOW8", syms)

    def test_min_move_pct_zero_allows_all(self):
        """Setting min_move_pct=0 lets everything through."""
        self._insert("BTCUSDT", rank_type="VOLUME", change="-0.5")
        syms = [i.symbol for i in self.repo.items_for_gemini(max_n=20, min_move_pct=0.0)]
        self.assertIn("BTCUSDT", syms)

    def test_negative_change_abs_used(self):
        """Negative large moves (losers) pass the threshold via abs()."""
        self._insert("BIGLOSER", rank_type="LOSER", change="-55.0")
        syms = [i.symbol for i in self.repo.items_for_gemini(max_n=20, min_move_pct=8.0)]
        self.assertIn("BIGLOSER", syms)

    # ------------------------------------------------------------------
    # Rule 2: max_candidates limit
    # ------------------------------------------------------------------
    def test_max_candidates_limit_respected(self):
        for i in range(15):
            self._insert(f"COIN{i:03d}", change="20.0")
        result = self.repo.items_for_gemini(max_n=5, min_move_pct=0.0)
        self.assertLessEqual(len(result), 5)

    # ------------------------------------------------------------------
    # Rule 2: sort order
    # ------------------------------------------------------------------
    def test_sort_best_rank_position_asc(self):
        """Lower rank number (= better rank) must come first."""
        self._insert("RANK3", rank_type="GAINER", position=3, change="50.0")
        self._insert("RANK1", rank_type="GAINER", position=1, change="20.0")
        self._insert("RANK2", rank_type="GAINER", position=2, change="10.0")
        result = self.repo.items_for_gemini(max_n=20, min_move_pct=0.0)
        syms = [i.symbol for i in result]
        self.assertLess(syms.index("RANK1"), syms.index("RANK2"))
        self.assertLess(syms.index("RANK2"), syms.index("RANK3"))

    def test_sort_abs_change_desc_within_same_rank(self):
        """Among same rank position, higher abs(change) must come first."""
        self._insert("LOW_CHG", position=2, change="10.0")
        self._insert("HIGH_CHG", position=2, change="40.0")
        result = self.repo.items_for_gemini(max_n=20, min_move_pct=0.0)
        syms = [i.symbol for i in result]
        self.assertLess(syms.index("HIGH_CHG"), syms.index("LOW_CHG"))

    def test_sort_quote_volume_desc_tiebreak(self):
        """With same rank + same change, higher volume must come first."""
        self._insert("LOW_VOL", position=5, change="20.0", volume="1000000.0")
        self._insert("HIGH_VOL", position=5, change="20.0", volume="9000000.0")
        result = self.repo.items_for_gemini(max_n=20, min_move_pct=0.0)
        syms = [i.symbol for i in result]
        self.assertLess(syms.index("HIGH_VOL"), syms.index("LOW_VOL"))

    # ------------------------------------------------------------------
    # NEW vs ACTIVE both show up together
    # ------------------------------------------------------------------
    def test_new_and_active_both_in_candidates(self):
        self._insert("ACTIVE_ONE", change="30.0", status="ACTIVE")
        self._insert("NEW_ONE", change="20.0", status="NEW")
        syms = [i.symbol for i in self.repo.items_for_gemini(max_n=20)]
        self.assertIn("ACTIVE_ONE", syms)
        self.assertIn("NEW_ONE", syms)


class TestGeminiReviewCandidateCount(unittest.TestCase):
    """PR #33 — SKIPPED response must contain candidate_count."""

    def setUp(self):
        self._db = tempfile.NamedTemporaryFile(suffix=".db", delete=False)
        self._svc = LeaderboardWatchService(db_path=self._db.name)
        # pre-populate pool with high-move coins
        for sym, rtype, chg in [
            ("COINA", "GAINER", "30.0"),
            ("COINB", "LOSER", "-25.0"),
            ("COINC", "GAINER", "15.0"),
        ]:
            item = WatchItem(
                watch_id=f"id-{sym}", symbol=sym,
                first_seen_at=_now(), last_seen_at=_now(),
                first_rank_type=rtype, latest_rank_type=rtype,
                best_rank_position=1, latest_rank_position=1,
                first_change_24h=chg, latest_change_24h=chg,
                quote_volume="5000000.0", appearances_24h=1, status="NEW",
            )
            self._svc._repo.upsert_item(item)
    ...
    def test_skipped_key_missing_includes_candidate_count(self):
        """Without GEMINI_API_KEY the response must still include candidate_count."""
        with patch.dict("os.environ", {}, clear=True):
            os.environ.pop("GEMINI_API_KEY", None)
            result = self._svc.gemini_review(max_candidates=20, min_gemini_move_pct=8.0)
        self.assertEqual(result["status"], "SKIPPED")
        self.assertEqual(result["reason"], "gemini_api_key_missing")
        self.assertIn("candidate_count", result)
        self.assertGreater(result["candidate_count"], 1)

    def test_candidate_count_reflects_min_move_filter(self):
        """candidate_count drops when min_gemini_move_pct filters out low-move coins."""
        # Add a low-move coin — should NOT count toward candidates at 8%
        low_item = WatchItem(
            watch_id="id-LOWMOVE", symbol="LOWMOVE",
            first_seen_at=_now(), last_seen_at=_now(),
            first_rank_type="VOLUME", latest_rank_type="VOLUME",
            best_rank_position=1, latest_rank_position=1,
            first_change_24h="2.0", latest_change_24h="2.0",
            quote_volume="50000000.0", appearances_24h=1, status="NEW",
        )
        self._svc._repo.upsert_item(low_item)

        with patch.dict("os.environ", {}, clear=True):
            os.environ.pop("GEMINI_API_KEY", None)
            result_strict = self._svc.gemini_review(max_candidates=20, min_gemini_move_pct=8.0)
            result_loose = self._svc.gemini_review(max_candidates=20, min_gemini_move_pct=0.0)

        # strict: LOWMOVE excluded
        self.assertLess(result_strict["candidate_count"], result_loose["candidate_count"])

    def test_candidate_count_zero_when_no_qualifying_items(self):
        """If pool only has low-move VOLUME coins, candidate_count = 0."""
        import os
        db2 = tempfile.NamedTemporaryFile(suffix=".db", delete=False)
        svc2 = LeaderboardWatchService(db_path=db2.name)
        low = WatchItem(
            watch_id="id-BTC", symbol="BTCUSDT",
            first_seen_at=_now(), last_seen_at=_now(),
            first_rank_type="VOLUME", latest_rank_type="VOLUME",
            best_rank_position=1, latest_rank_position=1,
            first_change_24h="-0.5", latest_change_24h="-0.5",
            quote_volume="999999999.0", appearances_24h=1, status="NEW",
        )
        svc2._repo.upsert_item(low)
        try:
            with patch.dict("os.environ", {}, clear=True):
                os.environ.pop("GEMINI_API_KEY", None)
                result = svc2.gemini_review(max_candidates=20, min_gemini_move_pct=8.0)
        finally:
            svc2.close()
            os.unlink(db2.name)
        self.assertIn("candidate_count", result)
    ...
    def test_min_gemini_move_pct_cli_default(self):
        """CLI must expose --min-gemini-move-pct with default 8.0."""
        from binance_ai_trader.entrypoints.cli import build_parser
        parser = build_parser()
        args = parser.parse_args(["leaderboard-watch", "gemini-review"])
        self.assertEqual(args.min_gemini_move_pct, 8.0)

    def test_min_gemini_move_pct_cli_override(self):
        """CLI --min-gemini-move-pct must be overridable."""
        from binance_ai_trader.entrypoints.cli import build_parser
        parser = build_parser()
        args = parser.parse_args(["leaderboard-watch", "gemini-review",
                                   "--min-gemini-move-pct", "5.0"])
        self.assertEqual(args.min_gemini_move_pct, 5.0)
