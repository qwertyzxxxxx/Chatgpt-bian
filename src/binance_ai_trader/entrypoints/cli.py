...
from binance_ai_trader.notifications import TelegramClient, format_daily_top3
...
def build_parser() -> argparse.ArgumentParser:
    ...
    telegram_test = subparsers.add_parser(
        "telegram-test", help="send a safe Telegram connectivity test from environment settings"
    )
    telegram_test.add_argument("--timeout", type=float, default=10.0)
    telegram_test.add_argument(
        "--log-level", choices=("DEBUG", "INFO", "WARNING", "ERROR"), default="INFO"
    )

    run_loop = subparsers.add_parser(
        "run-loop", aliases=["runner"], help="run the fault-isolated Reserved VM scheduler"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not arguments or arguments[0] not in {"scan", "regime", "sectors", "backtest", "evaluate", "strategies", "auto-research", "auto_research", "paper-simulate", "daily-report", "telegram-test", "run-loop", "runner", "health", "capital", "space", "walk-forward", "collect-history", "-h", "--help"}:
    ...
    if args.command in {"run-loop", "runner"}:
        return _run_loop(args)
    if args.command == "telegram-test":
        return _telegram_test(args.timeout)
...
def _daily_report(database: Path, report_date: date | None) -> int:
    ...
    try:
        ...
        repository.close()
    notification = TelegramClient.from_env().send(format_daily_top3(payload))
    logging.getLogger(__name__).info(
        "Daily Telegram notification %s: %s", notification.status, notification.detail
    )
    print(json.dumps(payload, separators=(",", ":"), sort_keys=True))
    return 0


def _telegram_test(timeout: float) -> int:
    result = TelegramClient.from_env(timeout).send(
        "Binance AI Trader Telegram test. Research only; no orders are placed."
    )
...
def _run_loop(args: argparse.Namespace) -> int:
    ...
    tasks = default_tasks(
        ...
        collect_history=lambda: invoke([
            "collect-history", "--days", str(args.history_days),
            "--database", str(database), "--config", str(args.config),
            "--sectors-config", str(args.sectors_config), "--base-url", args.base_url,
            "--timeout", str(args.timeout), "--max-retries", str(args.max_retries),
            "--request-pause", str(args.history_request_pause), "--log-level", args.log_level,
        ]),
        health=lambda: invoke(["health", "--database", str(database)]),
    )
    repository = MarketDataRepository(database)
    try:
