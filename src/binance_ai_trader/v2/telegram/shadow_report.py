...
Time fields are all computed in real-time from created_at / filled_at /
closed_at stored in the DB.  No derived columns are read for display.
"""
...
# ---------------------------------------------------------------------------
# Time helpers — all real-time, no stored duration fields
# ---------------------------------------------------------------------------
...
def _fmt_delta(seconds: int) -> str:
    """Format a non-negative duration in seconds as 'XhYYm' or 'Ym'."""
...
def _elapsed_since(iso: str | None) -> str:
    """Duration from *iso* to now (UTC). Used for pending wait and hold time."""
    dt = _parse_iso(iso)
    if dt is None:
        return "—"
    delta = datetime.now(UTC) - dt
    return _fmt_delta(int(delta.total_seconds()))


def _duration_between(start_iso: str | None, end_iso: str | None) -> str:
    """Duration from *start_iso* to *end_iso*. Used for settled hold time."""
    start = _parse_iso(start_iso)
    end   = _parse_iso(end_iso)
    if start is None or end is None:
        return "—"
    return _fmt_delta(int((end - start).total_seconds()))
...
def _pending_section(orders: list[V2PaperOrder]) -> str:
    ...
    rows = [f"\n【当前挂单（Pending）】\n共 {len(orders)} 笔"]
    for o in orders:
        # wait = time since order was created (real-time)
        wait = _elapsed_since(o.created_at)
        rows.append(
            f"  {o.symbol} {o.direction}\n"
            f"  Entry={o.entry}  等待={wait}  到期={_short_dt(o.expires_at)}"
        )
    return "\n".join(rows)
...
def _positions_section(
    ...
    for o in orders:
        ...
        # hold = time since fill (real-time, from filled_at to now)
        hold = _elapsed_since(o.filled_at)
        rows.append(
            f"  {o.symbol} {o.direction}\n"
...
def _recent_settled_section(orders: list[V2PaperOrder]) -> str:
    ...
    for o in orders:
        ...
        # hold duration = filled_at → closed_at (real-time calculation)
        dur = _duration_between(o.filled_at, o.closed_at)
