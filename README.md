...
The current implementation provides a read-only Binance USDⓈ-M Futures public-data layer, deterministic scoring engine, and deterministic regime-directed LONG/SHORT signal engine. It contains no Binance authentication, account, order, AI-model, or web functionality. Optional Telegram notifications use only environment-provided bot credentials and never affect trading analysis.
...
## P1 runtime automation and optional Telegram

The restart-safe scheduler can be started with either command name:

```bash
PYTHONPATH=src python -m binance_ai_trader runner --database data/market_data.db
# Backward-compatible name:
PYTHONPATH=src python -m binance_ai_trader run-loop --database data/market_data.db
```

The single-instance runner records every task in SQLite and isolates failures. It schedules `scan`,
`evaluate`, paper simulation, and `health` every 15 minutes; the daily Top 3 report at UTC 00:05;
resumable `collect-history` at UTC 00:20; and the existing research task every six hours. Restarting
uses the latest `runner_events` timestamps, so completed schedules are not immediately duplicated.

Telegram is optional and disabled by default. Configure environment variables outside source control:
...
If Telegram is disabled or either credential is missing, the command returns JSON with
`"status":"SKIPPED"`; the runner and daily report continue normally. Telegram uses Python `urllib`
only. No Binance API key is used, and no account or order endpoint exists.

Database health is available as stable JSON:

```bash
PYTHONPATH=src python -m binance_ai_trader health --database data/market_data.db
```

The output includes `database_integrity` from SQLite `PRAGMA quick_check`, database size, latest scan,
regime, signal count, runner error, paper equity, and aggressive-paper eligibility.
